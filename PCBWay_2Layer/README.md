# PCBWay 2-Layer FR-4 KiCad Template

KiCad project template matching **PCBWay's default online-quote settings** for a
standard FR-4 2-layer board -- the pre-selected, no-surcharge options on
...
Copy the `PCBWay_2Layer/` directory into your user template path, then
...
| Core  | FR-4 TG150, Dk 4.6  | 1.510 mm  |
| B.Cu  | Cu 1 oz             | 0.035 mm  |

No impedance control on 2-layer (single core, no reference plane). PCBWay's standard multilayer uses **1 oz inner copper**, so inner and outer
